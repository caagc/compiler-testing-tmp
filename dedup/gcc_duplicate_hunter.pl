...
my $HEADER_SELF = "";
# my $GCC_SRC = "$ENV{GCC_SRC}";

my $HEADER = "-I$CSMITH_HOME/runtime $HEADER_SELF";
my $CYGWIN_HEADER = "-I`cygpath -d ${CSMITH_HOME}/runtime`";
my $COMPILE_OPTIONS = "";
my @COMPILERS;
my @gccs = ('gcc-4.4.0','gcc-4.5.0','gcc-4.6.0','gcc-4.7.0','gcc-4.8.0', 'gcc-4.9.0','gcc-5.1.0', 'gcc-6.1.0','gcc-7.1.0','gcc-8.1.0', 'gcc-9.1.0','gcc-10.1.0','gcc-11.1.0','gcc-12.1.0'); # needs to be set.
my @exist_gccs;
my @fail_gccs;
our $bufdir;
...
sub test_one ($$$$) {
    my ($base_version,$cfile,$rev,$testbranch) = @_;
    @fail_gccs = getfailgccs();
    if (any {$_ eq $rev} @fail_gccs) {
        print "test on a fail gcc revision, continue.\n";
        return -1;
    }
    genCOMPILERS($base_version,$rev,$testbranch)==0 or return -1;
    my $seed;
    my $filesize;
    my $good_ = -1;
...
sub get_passversion_branchname($) {
    my ($gcc_version) = @_;
    my $firstsign;
    if (length($gcc_version) == 9) {
        $firstsign = substr($gcc_version,4,1);
    } else {
        $firstsign = substr($gcc_version,4,2);
    }
...
sub installRevision($$$) {
	my ($base_version,$rev,$testbranch) = @_;
    ...
    # chdir($GCC_SRC) or exit -1;
    # my $cmd2download = "git switch $rev; cp -r $GCC_SRC $srchome;";
    ...
    my $cmd2config;
    if ($base_version eq "") {
        $cmd2config = "CC=$USED_GCC/bin/gcc CXX=$USED_GCC/bin/g++ ./configure LDFLAGS=-Wl,--no-as-needed --prefix=$installhome --with-gmp=$ENV{'GMP_INSTALL'} --with-mpfr=$ENV{'MPFR_INSTALL'} --with-mpc=$ENV{'MPC_INSTALL'} --enable-languages=c,c++ --disable-multilib --disable-bootstrap --disable-nls 2>&1 >/dev/null";
    } else {
        $cmd2config = "CC=$GCC_INSTALL_PATH/$base_version/bin/gcc CXX=$GCC_INSTALL_PATH/$base_version/bin/g++ ./configure LDFLAGS=-Wl,--no-as-needed --prefix=$installhome --with-gmp=$ENV{'GMP_INSTALL'} --with-mpfr=$ENV{'MPFR_INSTALL'} --with-mpc=$ENV{'MPC_INSTALL'} --enable-languages=c,c++ --disable-multilib --disable-bootstrap --disable-nls 2>&1 >/dev/null";
    }
	my $cmd2make_install = "make -j8 && make install";
    print $cmd2config, "\n";
    ...
    chdir("$WORK_HOME") or return -1;
	return 0;
}

sub installRelease($$) {
    my ($base_version,$rev) = @_;
    my $gcc_home = $ENV{'GCC_HOME'};
    my $srchome = "$ENV{'GCC_HOME'}/$rev";
    my $releasename = $rev;
    $releasename =~ s/\-/\_/g;
    $releasename =~ s/\./\_/g;
    ...
    my $cmd2download = "wget https://mirrors-i.tuna.tsinghua.edu.cn/sourceware/gcc/releases/$rev/$rev.tar.gz -P $gcc_home 2>&1 1>/dev/null;
    tar xvf $gcc_home/$rev.tar.gz -C $gcc_home 2>&1 1>/dev/null; rm -rf $gcc_home/$rev.tar.gz";
    my $cmd2config;
    if ($base_version eq "") {
        $cmd2config = "CC=$USED_GCC/bin/gcc CXX=$USED_GCC/bin/g++ ./configure LDFLAGS=-Wl,--no-as-needed --prefix=$installhome --with-gmp=$ENV{'GMP_INSTALL'} --with-mpfr=$ENV{'MPFR_INSTALL'} --with-mpc=$ENV{'MPC_INSTALL'} --enable-languages=c,c++ --disable-multilib --disable-bootstrap --disable-nls 2>&1 >/dev/null";
    } else {
    ...
    my $cmd2make_install = "make -j8 2>&1 1>/dev/null && make install 2>&1 1>/dev/null";
    print $cmd2config, "\n";
    system($cmd2download)==0 or return -1;
    chdir($srchome) or return -1;
    system($cmd2config)==0 or return -1;
...
sub genCOMPILERS($$$) {
    my ($base_version,$sign,$testbranch) = @_;
    my $GCC_INSTALL_PATH = $ENV{"GCC_INSTALL_PATH"};
    if(index($sign, 'gcc') != -1) {
        ...
        if(-d "$GCC_INSTALL_PATH/$sign") {
            ...
            # }
        	if(installRelease($base_version,$sign)!=0){
                print "install $sign failed!\n";
                return -1;
            } 
            print "install $sign success!\n";
		}
    } else {
        ...
        if(-d "$GCC_INSTALL_PATH/$sign") {
            ...
            print "not exist gcc version: $sign\n";
            # @exist_gccs = getexistgccs();
            # if (scalar(@exist_gccs) > 500) {
            #    my $rev2del = $exist_gccs[-1]; 
	        #    my $srchome2del = "$ENV{'GCC_HOME'}/$rev2del";
            ...
            #    system("rm -rf $installhome2del");
            # }
            if (installRevision($base_version,$sign,$testbranch)!=0) {
                print "install $sign failed!\n";
                # exit -1;
                return -1;
            } 
    ...
    my $exepath = "$GCC_INSTALL_PATH/$sign/bin/gcc";
...
# sub getboundary($$) {
#     my ($fail_v, $pass_v) = @_;
#     my $fail_tag = "releases/" . $fail_v;
#     my $pass_tag = "releases/" . $pass_v;
#     chdir($GCC_SRC);
#     # my $cmd = "git log --pretty=format:\"%H\" $fail_tag..$pass_tag";
#     # get all revisions to be tested including the fail_tag
    
#     chdir("$WORK_HOME/dedup");
# }

my %revs2tests = {};
sub getRevs2Test($$$) {
    my ($start_rev, $end_rev, $testbranch) = @_;
    my $key = "$start_rev:$end_rev:$testbranch";
    if (exists $revs2tests{$key}) {
        my $ref = $revs2tests{$key};
        return @$ref;
    }
    my @revs2test;
...
sub binaryfind($$$) {
    ...
    # chdir($gcc_svnbase);
    my @revs2test = getRevs2Test($start_rev,$end_rev,$testbranch);
    # my $cmd = "svn log -r $start_rev:$end_rev -v svn://gcc.gnu.org/svn/gcc/branches/$testbranch| grep -o \'r[0-9]\\{6\\}\\s|\' | grep -o \'r[0-9]\\{6\\}\'";
    # print "$cmd\n";
    # open(EXEC, '-|', $cmd) 
    #     or return -1;
    ...
    my $test_version = $start_v;
    if (test_one($test_version,$cfilepath,$revs2test[0],$testbranch)==0) {
        return $revs2test[0];
    }
    print "revs2test[-1] is: $revs2test[-1]\n";
    if (test_one($test_version,$cfilepath,$revs2test[-1],$testbranch)==1) {
        return -1;
    }
    ...
    while($left < $right) {
        print "left is: $left\nmid is: $mid\nright is: $right\n";
        my $test_res = test_one($test_version,$cfilepath,$revs2test[$mid],$testbranch);
        if($test_res==0) {
            print "TEST PASSED!\n";
            $right = $mid - 1;
...
my $start_test;
if ($nargs == 3) {
    $start_test = "$WORK_HOME/bugs/" . $ARGV[2];
    print "start_test is: $start_test\n";
} else {
    $start_test = "";
}
...
@exist_gccs = getexistgccs();
my $path = "$WORK_HOME";
my @full_pathes = File::Find::Rule->file->name('*.c')->in($path);
my @full_pathes = sort {
...
# install gccs using installRelease.
my $last_version = ""
foreach(@gccs) {
    my $rev = $_;
    installRelease($last_version,$rev);
    $last_version = $rev;
}
# my @existcfile;
# if (-e $dataname) {
